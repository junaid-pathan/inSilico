...
export default function SimulatorPage() {
  // We use a global Context Provider (useSimulator) to store all our state.
  // This ensures that if the user navigates away to the Home page and comes back,
  // their simulation charts and patient data aren't lost!
  const {
    patient,
    isUploaded,
    setIsUploaded,
  ...
  // Sends the patient's current BRFSS stats to the backend to get a 'Baseline Score'.
  // This represents their sickness level BEFORE taking the simulated drug.
  ...
  // Once the user uploads a drug PDF, this function packages the PDF data (moa) 
  // along with the patient's baseline risk, and pings the backend to run the full simulation.
  // It then saves the results to the global context so the graphs can render!
...
// RENDER PHASE 1: The Patient Intake Wizard
// If no simulation has been run yet, show the guided questionnaire.
                ...
                {featureDeltas.map((d: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`text-right font-mono text-xs font-bold ${good ? "text-[oklch(0.8_0.18_160)]" : "text-accent"
                          }`}
                      >
                        {d.delta > 0 ? "+" : ""}
                        {d.delta.toFixed(2)}
