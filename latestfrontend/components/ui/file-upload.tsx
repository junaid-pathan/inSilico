...
export default function FileUpload({ onUploadComplete }: { onUploadComplete?: (data?: any) => void }) {
  ...
  const simulateUpload = async (id: string, file?: File) => {
    ...
    try {
      ...
      console.log(
        `%c[InSilico] gamma ${Number(data?.gamma ?? 0).toFixed(2)} via ${usedFormula ? "CALIBRATED FORMULA ✅" : "LLM GUESS (fallback) 🟡"}`,
        `font-weight: bold; color: ${usedFormula ? "#166534" : "#92400e"}; font-size: 13px;`
      );
      if (!usedFormula) {
        console.log("[InSilico] fallback_reason:", data?.fallback_reason);
      }
      console.log("[InSilico] extracted endpoints:", data?.evidence?.endpoints_extracted);
      console.log("[InSilico] full parsed response:", data);
